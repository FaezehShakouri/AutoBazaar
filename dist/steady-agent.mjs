...
export async function decide({self,products,suppliers,day,weather}){
  ...
  for(const p of products){
    ...
    if(held<16){const quantity=32-held,cost=Math.floor((p.cost*Math.round(supplier.multiplier*100)*(quantity>=24?92:100)+5000)/10000)*quantity;if(cost<=available){orders.push({product:p.id,supplier:supplier.id,quantity});available-=cost;}}
  }
