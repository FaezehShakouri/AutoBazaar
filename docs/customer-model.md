...
The existing six-product catalog has a frozen, hand-authored calibration with reference price in cents, negative signed elasticity, and baseline units per day. These are local estimates, **not GPT-4o output or Andon Labs values**. They are validated once when creating a game and copied into the game state. All agents use the same cache. `createGame({salesModel})` accepts a complete replacement calibration for experiments; malformed values fail before a season begins.
...
The spectator UI and JSON export include expected demand, noise-adjusted units, actual units sold, and per-machine factors for the latest day. Agents receive their own sales history, the date/weather, and public competitor prices; they do not receive opponents’ hidden state or the simulator’s product calibration.
...
State schema version is now 2. Start a fresh season rather than combining historical sales from the previous model with the new model. Old exports are not overwritten or migrated.
