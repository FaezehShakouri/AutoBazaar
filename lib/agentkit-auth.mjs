...
import {houseIdentity} from './season-participation.mjs';
...
export class AgentKitGate {
  ...
  async challenge(url,method,rawBody){
    const options={domain:new URL(url).hostname,resourceUri:url,network:this.store.arc?[`eip155:${this.config.chainId}`,`eip155:${this.store.arc.metadata.chainId}`]:`eip155:${this.config.chainId}`,expirationSeconds:120,mode:{type:'free'},statement:`AutoBazaar: authorize ${method} ${new URL(url).pathname}. Request SHA-256: ${bodyDigest(rawBody)}. ${this.store.arc?'This season uses test USDC on Arc Testnet. Financial actions require separate wallet authorization.':'Game balances are simulated.'}`};
  ...
  async authenticateWallet(header,url,method,rawBody){
    ...
    const signatureNetworks=[`eip155:${this.config.chainId}`,...(this.store.arc?[`eip155:${this.store.arc.metadata.chainId}`]:[])];
    if(!signatureNetworks.includes(payload.chainId)||!['eip191','eip1271'].includes(payload.type)||payload.signatureScheme&&!['eip191','eip1271'].includes(payload.signatureScheme))reject(401,'Unsupported agent signature network or type.');
    ...
    try{
      ...
      else if(payload.chainId===`eip155:${this.store.arc?.metadata.chainId}`){
        try{signatureValid=await this.store.arc.verifyMessage({address:payload.address,message:formatSIWEMessage(payload,payload.address),signature:payload.signature});}
        catch(error){console.error('Arc signature check:',error.name,(error.shortMessage||error.message||'Unavailable').split('\n')[0].replace(/https?:\/\/\S+/g,'[RPC]'));reject(503,'Arc wallet signature verification is temporarily unavailable. Retry shortly.');}
      }
      else {
        const checked=await verifyAgentkitSignature(payload,{rpcUrl:payload.chainId===`eip155:${this.store.arc?.metadata.chainId}`?this.store.arc.config.rpcUrl:this.config.rpcUrl});
        signatureValid=checked.valid;
        if(!checked.valid&&/HTTP request failed|RPC Request failed|timed? out|fetch failed|rate.limit|status.*(?:429|50[234])/i.test(checked.error||''))reject(503,'Wallet signature verification is temporarily unavailable. Retry shortly.');
      }
    }catch(error){if(error instanceof SeasonError)throw error;}
  ...
  async authenticate(header,url,method,rawBody){
    const {address}=await this.authenticateWallet(header,url,method,rawBody);
    // Only three explicitly configured test house wallets may skip human lookup.
    // They must still sign the exact AgentKit challenge and every Arc permission.
    if(this.store.isHouse(address))return {address,humanId:houseIdentity(address),participantKind:'house'};
    const humanId=await this.book.lookupHuman(address);
    if(!humanId)reject(403,`This wallet is not registered in World AgentBook. Run: npx @worldcoin/agentkit-cli@0.2.0 register ${address}`);
    return {address,humanId,participantKind:'human'};
  }
}
