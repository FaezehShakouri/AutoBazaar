import {createHmac,randomBytes,randomInt} from 'node:crypto';
import {seasonParticipation,houseIdentity} from './season-participation.mjs';
import {createGame,registerAgent,startRound,prepareDay,settleDay,observation,validateDecision,PRODUCTS,SUPPLIERS} from '../dist/engine.js';

export class SeasonError extends Error {
  constructor(status,message){super(message);this.status=status;}
}
const fail=(status,message)=>{throw new SeasonError(status,message);};
const encode=JSON.stringify;
const namePattern=/^[\p{L}\p{N} ._-]{1,24}$/u;

// All state transitions are synchronous SQLite transactions. Multiple requests
// or processes using this same database cannot claim the same seat or day.
export class SeasonStore {
  constructor({db,turnMs=180000,intermissionMs=30000,now=Date.now,bookScope='worldchain:480:canonical',allowRetiredSandboxMigration=false,participation=seasonParticipation(),allowEmptyDemoMigration=false,allowSingleHumanDemoMigration=false}={}){
    if(!db)throw Error('A season database is required.');
    for(const [key,value] of Object.entries({turnMs,intermissionMs}))if(!Number.isInteger(value)||value<1)throw Error(`Invalid ${key}.`);
    this.db=db;this.now=now;this.turnMs=turnMs;this.intermissionMs=intermissionMs;this.bookScope=bookScope;this.participation=participation;
    this.db.exec(`
      CREATE TABLE IF NOT EXISTS settings(key TEXT PRIMARY KEY,value TEXT NOT NULL);
      CREATE TABLE IF NOT EXISTS seasons(id INTEGER PRIMARY KEY AUTOINCREMENT,created_at INTEGER NOT NULL,finished_at INTEGER,state TEXT NOT NULL,morning TEXT,deadline INTEGER,next_day_at INTEGER);
      CREATE TABLE IF NOT EXISTS entrants(season_id INTEGER NOT NULL,slot TEXT NOT NULL,address TEXT NOT NULL,human_key TEXT NOT NULL,joined_at INTEGER NOT NULL,PRIMARY KEY(season_id,slot),UNIQUE(season_id,address),UNIQUE(season_id,human_key));
      CREATE TABLE IF NOT EXISTS decisions(season_id INTEGER NOT NULL,day INTEGER NOT NULL,slot TEXT NOT NULL,decision TEXT NOT NULL,submitted_at INTEGER NOT NULL,PRIMARY KEY(season_id,day,slot));
      CREATE TABLE IF NOT EXISTS house_requests(season_id INTEGER PRIMARY KEY,attempt INTEGER NOT NULL,status TEXT NOT NULL,requested_by TEXT NOT NULL,updated_at INTEGER NOT NULL,code TEXT,lease TEXT,lease_holder TEXT,lease_until INTEGER);
      CREATE TABLE IF NOT EXISTS challenges(nonce TEXT PRIMARY KEY,info TEXT NOT NULL,method TEXT NOT NULL,body_hash TEXT NOT NULL,expires INTEGER NOT NULL,consumed INTEGER NOT NULL DEFAULT 0);
      CREATE INDEX IF NOT EXISTS challenge_expiry ON challenges(expires);`);
    this.db.prepare('INSERT OR IGNORE INTO settings VALUES (?,?)').run('identity_salt',randomBytes(32).toString('hex'));
    this.salt=this.db.prepare('SELECT value FROM settings WHERE key=?').get('identity_salt').value;
    this.transaction(()=>{
      const scope=this.db.prepare('SELECT value FROM settings WHERE key=?').get('book_scope');
      if(scope&&scope.value!==bookScope){
        const jobs=this.db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='arc_jobs'").get();
        const empty=!this.db.prepare('SELECT COUNT(*) AS n FROM entrants').get().n&&(!jobs||!this.db.prepare('SELECT COUNT(*) AS n FROM arc_jobs').get().n);
        if(!allowRetiredSandboxMigration||!scope.value.startsWith('world-id-sandbox:')||bookScope!=='production:480:0xa23ab2712ea7bba896930544c7d6636a96b944da'||!empty)throw Error('This database belongs to another identity environment. Use a separate escrow and database for funded seasons.');
        // Only the explicitly retired, completely empty app registry may migrate.
        // Old proofs and challenges cannot authorize AgentBook entry.
        this.db.prepare('UPDATE settings SET value=? WHERE key=?').run(bookScope,'book_scope');
        this.db.exec('DELETE FROM challenges');
      }
      this.db.prepare('INSERT OR IGNORE INTO settings VALUES (?,?)').run('book_scope',bookScope);
      const participationJson=JSON.stringify(participation),savedParticipation=this.db.prepare('SELECT value FROM settings WHERE key=?').get('participation');
      const priorParticipation=savedParticipation?.value||JSON.stringify(seasonParticipation());
      if(priorParticipation!==participationJson){
        const jobs=this.db.prepare("SELECT 1 FROM sqlite_master WHERE type='table' AND name='arc_jobs'").get();
        const empty=!this.db.prepare('SELECT COUNT(*) AS n FROM entrants').get().n&&(!jobs||!this.db.prepare('SELECT COUNT(*) AS n FROM arc_jobs').get().n);
        const newDatabase=!this.db.prepare('SELECT COUNT(*) AS n FROM seasons').get().n;
        // The owner may turn a waiting one-human lobby into the hackathon demo.
        // Its verified identity, deposit and all financial history remain unchanged.
        const entrants=this.db.prepare('SELECT * FROM entrants').all();
        const singleHuman=allowSingleHumanDemoMigration&&JSON.parse(priorParticipation).mode==='competition'&&participation.mode==='demo'&&entrants.length===1&&!participation.houseAddresses.includes(entrants[0].address)&&this.get(entrants[0].season_id).state.phase==='lobby'&&this.db.prepare('SELECT COUNT(*) AS n FROM seasons').get().n===1&&(!jobs||!this.db.prepare("SELECT 1 FROM arc_jobs WHERE kind!='join' OR status!='confirmed' OR address!=? OR season_id!=? LIMIT 1").get(entrants[0].address,entrants[0].season_id));
        if(!singleHuman&&(!empty||(!newDatabase&&!allowEmptyDemoMigration)))throw Error('Participation settings are pinned. Use an isolated escrow/database for funded seasons; an empty lobby requires explicit migration.');
        this.db.exec('DELETE FROM challenges');
      }
      this.db.prepare('INSERT OR REPLACE INTO settings VALUES (?,?)').run('participation',participationJson);
      this.ensureLobby();
    });
  }
  transaction(fn){if(this.db.transactionSync)return this.db.transactionSync(fn);this.db.exec('BEGIN IMMEDIATE');try{const result=fn();this.db.exec('COMMIT');return result;}catch(error){this.db.exec('ROLLBACK');throw error;}}
  close(){this.db.close();}
  get(id){
    if(!Number.isSafeInteger(Number(id))||Number(id)<1)fail(404,'Season not found.');
    const row=this.db.prepare('SELECT * FROM seasons WHERE id=?').get(Number(id));if(!row)fail(404,'Season not found.');
    return {...row,state:JSON.parse(row.state),morning:row.morning?JSON.parse(row.morning):null};
  }
  save(s){this.db.prepare('UPDATE seasons SET state=?,morning=?,deadline=?,next_day_at=?,finished_at=? WHERE id=?').run(encode(s.state),s.morning?encode(s.morning):null,s.deadline??null,s.next_day_at??null,s.finished_at??null,s.id);}
  ensureLobby(){
    const open=this.db.prepare("SELECT id FROM seasons WHERE json_extract(state,'$.phase')='lobby' ORDER BY id LIMIT 1").get();if(open)return open.id;
    const state=createGame({mode:'remote',seed:randomInt(0,4294967296),startDate:new Date(this.now()).toISOString().slice(0,10)});
    const result=this.db.prepare('INSERT INTO seasons(created_at,state) VALUES (?,?)').run(this.now(),encode(state));
    const id=Number(result.lastInsertRowid);state.round=id;this.db.prepare('UPDATE seasons SET state=? WHERE id=?').run(encode(state),id);return id;
  }
  isHouse(address){return this.participation.mode==='demo'&&this.participation.houseAddresses.includes(address.toLowerCase());}
  participantKind(address){return this.isHouse(address)?'house':'human';}
  participantBrief(address){return this.isHouse(address)?'Demo house agent. Use your stated strategy to compete with one World ID-backed entrant and two house agents. This is an operator-owned test wallet, not a separately verified human.':'Independent remote agent acting on behalf of a verified human.';}
  assertSeatAvailable(id,address,reserved=[]){
    if(this.participation.mode!=='demo')return;
    const peers=[...this.entries(Number(id)),...reserved].filter(e=>e.address!==address.toLowerCase());
    const house=this.isHouse(address),limit=house?3:1;
    if(peers.filter(e=>this.isHouse(e.address)===house).length>=limit)fail(409,house?'All three demo house seats are reserved.':'This demo has one human seat; it is already occupied or awaiting payment.');
  }
  humanKey(id,humanId){return createHmac('sha256',this.salt).update(`${this.bookScope}:${id}:${humanId}`).digest('hex');}
  entries(id){return this.db.prepare('SELECT slot,address,joined_at FROM entrants WHERE season_id=? ORDER BY joined_at,slot').all(id);}
  summary(s){
    const entries=this.entries(s.id),submitted=s.morning?this.db.prepare('SELECT slot FROM decisions WHERE season_id=? AND day=?').all(s.id,s.morning.day).map(r=>r.slot):[];
    const sandbox=this.bookScope.startsWith('world-id-sandbox:');
    return {id:s.id,mode:this.participation.mode,participation:this.participation,houseRequest:this.houseRequest(s.id),seatCounts:{human:entries.filter(e=>!this.isHouse(e.address)).length,house:entries.filter(e=>this.isHouse(e.address)).length},identityEnvironment:sandbox?'world-id-sandbox':'agentbook',status:s.state.phase==='lobby'?'open':s.state.phase==='finished'?'finished':'running',day:s.state.day,decidingDay:s.morning?.day??null,deadline:s.deadline,nextDayAt:s.next_day_at,createdAt:s.created_at,finishedAt:s.finished_at,registered:entries.length,capacity:4,registrationStake:s.state.registrationStake,startingCash:s.state.startingCash,customerBudget:s.state.customerBudgetRemaining,turnMs:this.turnMs,intermissionMs:this.intermissionMs,submitted,
      entrants:entries.map(e=>{const a=s.state.agents.find(a=>a.id===e.slot);return {slot:e.slot,name:a.name,strategy:a.strategy,color:a.color,address:e.address,participantKind:this.participantKind(e.address),humanBacked:!sandbox&&!this.isHouse(e.address),sandboxVerified:sandbox,cash:a.cash,active:a.active};}),
      standings:s.state.phase==='finished'?s.state.agents.slice().sort((a,b)=>b.cash-a.cash||a.id.localeCompare(b.id)).map(a=>({slot:a.id,name:a.name,cash:a.cash,revenue:a.revenue,sold:a.sold,rank:1+s.state.agents.filter(b=>b.cash>a.cash).length})):[]};
  }
  list(){return this.db.prepare('SELECT id FROM seasons ORDER BY id DESC LIMIT 100').all().map(row=>this.summary(this.get(row.id)));}
  publicView(id){const s=this.get(id),state=structuredClone(s.state);delete state.rng;delete state.salesModel;for(const a of state.agents){a.memory='';const entry=this.entries(s.id).find(e=>e.slot===a.id);a.participantKind=entry?this.participantKind(entry.address):null;}return {season:this.summary(s),state};}
  authorize(id,identity){
    const entry=this.db.prepare('SELECT * FROM entrants WHERE season_id=? AND address=?').get(Number(id),identity.address.toLowerCase());
    if(!entry||entry.human_key!==this.humanKey(id,identity.humanId))fail(403,'This wallet and its currently verified human do not own a seat in this season.');
    return entry;
  }
  houseRequest(id){
    const row=this.db.prepare('SELECT attempt,status,updated_at,code,lease_until FROM house_requests WHERE season_id=?').get(Number(id));
    if(!row)return null;
    const messages={funds:'The host needs to replenish the house wallets with test USDC.',runner:'The house runner stopped. Retry to resume the same seats.',stopped:'The host runner was stopped. Retry when the host is available.'};
    return {attempt:row.attempt,status:row.status,updatedAt:row.updated_at,hostOnline:Boolean(row.lease_until>this.now()),message:messages[row.code]||null};
  }
  manageHouseAgents(id,identity,{operation='request',attempt,lease,status,code}={}){
    return this.transaction(()=>{
      const s=this.get(id),address=identity.address.toLowerCase(),now=this.now();
      if(this.participation.mode!=='demo')fail(409,'House agents are available only in demo seasons.');
      const row=this.db.prepare('SELECT * FROM house_requests WHERE season_id=?').get(s.id);
      if(operation==='request'){
        this.authorize(id,identity);
        if(this.isHouse(address))fail(403,'Only the registered human entrant can request house agents.');
        if(s.state.phase==='finished')fail(409,'This season has finished.');
        if(row&&row.status!=='failed')return this.houseRequest(id);
        if(!row&&s.state.phase!=='lobby')fail(409,'All house seats are already filled.');
        this.db.prepare("INSERT INTO house_requests(season_id,attempt,status,requested_by,updated_at) VALUES (?,?,'queued',?,?) ON CONFLICT(season_id) DO UPDATE SET attempt=excluded.attempt,status='queued',updated_at=excluded.updated_at,code=NULL,lease=NULL,lease_holder=NULL,lease_until=NULL").run(s.id,(row?.attempt||0)+1,address,now);
        return this.houseRequest(id);
      }
      if(!this.isHouse(address)||identity.humanId!==houseIdentity(address))fail(403,'Only an authenticated configured house wallet can run house requests.');
      if(!row||row.attempt!==attempt)fail(409,'The house request changed. Refresh before continuing.');
      if(operation==='claim'){
        if(s.state.phase==='finished')fail(409,'This season has finished.');
        if(!['queued','starting','playing'].includes(row.status)||row.lease_until>now)fail(409,'This house request is already claimed or needs a human retry.');
        const token=randomBytes(24).toString('hex');
        this.db.prepare("UPDATE house_requests SET status='starting',updated_at=?,code=NULL,lease=?,lease_holder=?,lease_until=? WHERE season_id=?").run(now,token,address,now+120000,s.id);
        return {...this.houseRequest(id),lease:token};
      }
      if(operation!=='heartbeat')fail(400,'Unknown house request operation.');
      if(!lease||row.lease!==lease||row.lease_holder!==address||row.lease_until<=now)fail(409,'The house runner lease expired or belongs to another runner.');
      if(!['starting','playing','failed','finished'].includes(status)||code&&!['funds','runner','stopped'].includes(code))fail(400,'Invalid house runner status.');
      if(status==='playing'&&this.entries(id).filter(e=>this.isHouse(e.address)).length!==3)fail(409,'All three house entries must confirm before reporting play.');
      if(status==='finished'&&s.state.phase!=='finished')fail(409,'The season is not finished.');
      const terminal=['failed','finished'].includes(status);
      this.db.prepare('UPDATE house_requests SET status=?,updated_at=?,code=?,lease_until=? WHERE season_id=?').run(status,now,code||null,terminal?0:now+120000,s.id);
      return this.houseRequest(id);
    });
  }
  join(id,identity,{name,strategy='Independent'}={}){
    if(typeof name!=='string'||!namePattern.test(name.trim()))fail(400,'Agent name must be 1–24 letters, numbers, spaces, dots, hyphens or underscores.');
    if(typeof strategy!=='string'||!namePattern.test(strategy.trim()))fail(400,'Strategy label must be 1–24 letters, numbers, spaces, dots, hyphens or underscores.');
    return this.transaction(()=>{
      const s=this.get(id),address=identity.address.toLowerCase(),humanKey=this.humanKey(id,identity.humanId);
      const existing=this.db.prepare('SELECT * FROM entrants WHERE season_id=? AND (address=? OR human_key=?)').all(s.id,address,humanKey);
      if(existing.some(e=>e.address===address&&e.human_key===humanKey))return {slot:existing.find(e=>e.address===address).slot,season:this.summary(s),alreadyJoined:true};
      if(existing.length)fail(409,'One agent per verified human per season. This human or wallet already has a seat.');
      this.assertSeatAvailable(s.id,address);
      if(s.state.phase!=='lobby')fail(409,'This season is full or has finished. Join an open season.');
      if(s.state.agents.some(a=>a.registered&&a.name.toLowerCase()===name.trim().toLowerCase()))fail(409,'That agent name is already used in this season.');
      const slot=s.state.agents.find(a=>!a.registered);slot.name=name.trim();slot.strategy=strategy.trim();slot.brief=this.participantBrief(address);slot.participantKind=this.participantKind(address);
      s.state=registerAgent(s.state,slot.id);
      this.db.prepare('INSERT INTO entrants VALUES (?,?,?,?,?)').run(s.id,slot.id,address,humanKey,this.now());
      if(s.state.agents.every(a=>a.registered)){s.state=startRound(s.state);this.beginDay(s);}
      this.save(s);this.ensureLobby();return {slot:slot.id,season:this.summary(s),alreadyJoined:false};
    });
  }
  beginDay(s){s.morning=prepareDay(s.state);s.deadline=this.now()+this.turnMs;s.next_day_at=null;}
  observe(id,identity){
    const s=this.get(id),entry=this.authorize(id,identity),submitted=s.morning?Boolean(this.db.prepare('SELECT 1 FROM decisions WHERE season_id=? AND day=? AND slot=?').get(s.id,s.morning.day,entry.slot)):false;
    return {season:this.summary(s),slot:entry.slot,submitted,observation:s.morning?observation(s.morning,entry.slot):null};
  }
  dashboard(id,identity){
    const status=this.observe(id,identity),s=this.get(id),index=s.state.agents.findIndex(a=>a.id===status.slot);
    const self=structuredClone(s.state.agents[index]);
    const decisions=this.db.prepare('SELECT day,decision,submitted_at FROM decisions WHERE season_id=? AND slot=? ORDER BY day DESC LIMIT 100').all(Number(id),status.slot).map(row=>({day:row.day,decision:JSON.parse(row.decision),submittedAt:row.submitted_at,status:row.day<=s.state.day?'settled':'submitted'}));
    return {...status,self,products:PRODUCTS,suppliers:SUPPLIERS,history:s.state.history.map(h=>({day:h.day,cash:h.cash[index]})),decisions,events:s.state.log.filter(e=>e.agent===status.slot||e.agent===null).slice(-120).reverse(),transactions:this.jobs?.(id)||[]};
  }
  submit(id,identity,{day,decision}={}){
    if(!Number.isInteger(day)||day<1)fail(400,'A positive decision day is required.');
    try{validateDecision(decision);}catch(e){fail(400,e.message);}
    return this.transaction(()=>{
      const s=this.get(id),entry=this.authorize(id,identity),prior=this.db.prepare('SELECT decision FROM decisions WHERE season_id=? AND day=? AND slot=?').get(s.id,day,entry.slot);
      if(prior){if(prior.decision!==encode(decision))fail(409,'A decision is already locked for this day.');return {accepted:true,duplicate:true,day,slot:entry.slot};}
      if(!s.morning||s.morning.day!==day||this.now()>=s.deadline)fail(409,'This decision window has closed. Fetch a fresh observation.');
      if(!s.morning.agents.find(a=>a.id===entry.slot).active)fail(409,'This machine is closed.');
      this.db.prepare('INSERT INTO decisions VALUES (?,?,?,?,?)').run(s.id,day,entry.slot,encode(decision),this.now());
      const count=this.db.prepare('SELECT COUNT(*) AS n FROM decisions WHERE season_id=? AND day=?').get(s.id,day).n;
      if(count===s.morning.agents.filter(a=>a.active).length)this.settle(s);
      return {accepted:true,duplicate:false,day,slot:entry.slot};
    });
  }
  settle(s){
    const decisions=Object.fromEntries(this.db.prepare('SELECT slot,decision FROM decisions WHERE season_id=? AND day=?').all(s.id,s.morning.day).map(r=>[r.slot,JSON.parse(r.decision)]));
    for(const a of s.morning.agents.filter(a=>a.active))if(!decisions[a.id])decisions[a.id]={prices:{},load:{},orders:[],rationale:'Decision deadline missed. Kept prices and placed no orders or restocks.',memory:a.memory};
    s.state=settleDay(s.morning,decisions);s.morning=null;s.deadline=null;
    s.next_day_at=s.state.phase==='finished'?null:this.now()+this.intermissionMs;
    if(s.state.phase==='finished')s.finished_at=this.now();this.save(s);
  }
  tick(){this.transaction(()=>{
    const due=this.db.prepare('SELECT id FROM seasons WHERE deadline<=? OR next_day_at<=?').all(this.now(),this.now());
    for(const row of due){const s=this.get(row.id);if(s.morning&&s.deadline<=this.now())this.settle(s);else if(s.next_day_at&&s.next_day_at<=this.now()){this.beginDay(s);this.save(s);}}
    this.db.prepare('DELETE FROM challenges WHERE expires<?').run(this.now()-60000);
  });}
  nextWake(){return this.db.prepare('SELECT MIN(time) AS time FROM (SELECT deadline AS time FROM seasons WHERE deadline IS NOT NULL UNION ALL SELECT next_day_at AS time FROM seasons WHERE next_day_at IS NOT NULL)').get().time;}
  saveChallenge(info,method,bodyHash){
    if(this.db.prepare('SELECT COUNT(*) AS n FROM challenges WHERE expires>?').get(this.now()).n>=10000)fail(503,'Verification is busy. Retry shortly.');
    this.db.prepare('INSERT INTO challenges(nonce,info,method,body_hash,expires) VALUES (?,?,?,?,?)').run(info.nonce,encode(info),method,bodyHash,Date.parse(info.expirationTime));
  }
  challenge(nonce){const row=this.db.prepare('SELECT * FROM challenges WHERE nonce=?').get(nonce);return row?{...row,info:JSON.parse(row.info)}:null;}
  consumeChallenge(nonce){const result=this.db.prepare('UPDATE challenges SET consumed=1 WHERE nonce=? AND consumed=0 AND expires>?').run(nonce,this.now());if(result.changes!==1)fail(401,'This challenge expired or was already used. Make a new request.');}
}
