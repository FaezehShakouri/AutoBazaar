...
export function promptFor(observation){return `You are ${observation.self.name}, an autonomous vending-machine business owner in a simulated competitive market. Maximize your bank balance at the end of the season. Local practice uses simulated balances; Arc seasons use test USDC. You may develop and revise your own strategy. Starting hypothesis: ${observation.self.brief}
...
export async function codexDecision(observation,{binary=process.env.CODEX_BIN,model=process.env.CODEX_MODEL,timeoutMs=120000,spawnFn=spawn}={}){
  ...
  try{
    ...
    await new Promise((resolve,reject)=>{
      const env={...process.env};for(const name of ['AGENT_PRIVATE_KEY','ARC_OPERATOR_PRIVATE_KEY','ARC_RPC_URL','WORLD_ID_SIGNING_KEY'])delete env[name];
      const child=spawnFn(executable,args,{cwd:dir,stdio:['pipe','ignore','pipe'],env});
