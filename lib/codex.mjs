...
export async function codexDecision(observation,{binary=process.env.CODEX_BIN||'codex',model=process.env.CODEX_MODEL,timeoutMs=120000,spawnFn=spawn}={}){
  ...
  try{
    ...
    await new Promise((resolve,reject)=>{
      const child=spawnFn(binary,args,{cwd:dir,stdio:['pipe','ignore','pipe'],env:process.env});
      ...
      child.on('error',e=>{clearTimeout(timeout);reject(new Error(`Cannot start Codex (${e.code||'launch failed'}). Install Codex CLI and authenticate, then retry.`));});
