...
import {ECONOMY} from '../dist/engine.js';
import {arcErrorMessage} from './arc.mjs';
import {seasonIdentity} from './season-identity.mjs';
...
export function createSeasonApi({store,config=agentBookConfig(),book,origin,worldEnv={},worldVerifyFetch}={}){
  const identityConfig=seasonIdentity(worldEnv,config),sandbox=new WorldSandbox({store,env:worldEnv,verifyFetch:worldVerifyFetch});
  ...
  return async function handle(request,{remoteAddress='local'}={}){
    try{
      ...
      if(['/api/world-id/enroll','/api/world-id/status'].includes(path)||path.startsWith('/api/world-id/session/'))return apiJson({error:'App-specific Sandbox enrollment has been retired. Register your wallet through World AgentBook.',registration:identityConfig.verification},410);
      if(path.startsWith('/api/world-id/')){
        ...
        let body;try{body=JSON.parse(raw);}catch(error){if(error instanceof SeasonError)throw error;throw new SeasonError(400,'Invalid JSON.');}
      ...
      if(path==='/api/seasons'&&request.method==='GET')return apiJson({seasons:store.list(),arc:store.arc?.metadata??null,verification:identityConfig.verification,participation:store.participation,rules:{agentsPerSeason:4,verifiedHumanSeats:store.participation.verifiedSeats,houseSeats:store.participation.houseSeats,registrationEnabled:true,oneAgentPerHuman:true,registrationStake:50000,startingCash:50000,currency:store.arc?'Arc test USDC':'simulated USDC',economy:ECONOMY,turnMs:store.turnMs}});
      const route=/^\/api\/seasons\/(\d+)\/(snapshot|join|observation|dashboard|decisions|withdraw|house-agents)$/.exec(path);
      if(!route)return apiJson({error:'Unknown season endpoint.'},404);
      const [,id,action]=route,method=action==='snapshot'||action==='observation'||action==='dashboard'?'GET':'POST';
      ...
      if(action==='join')return apiJson(await store.join(id,identity,body));
      if(action==='observation')return apiJson(store.observe(id,identity));
      if(action==='dashboard')return apiJson(store.dashboard(id,identity));
      if(action==='house-agents')return apiJson(store.manageHouseAgents(id,identity,body));
      if(action==='withdraw'){if(!store.withdraw)return apiJson({error:'This is a simulation; there are no onchain funds to withdraw.'},409);return apiJson(await store.withdraw(id,identity));}
      return apiJson(await store.submit(id,identity,body));
    }catch(error){if(!(error instanceof SeasonError))console.error('Season API error:',arcErrorMessage(error));return apiJson({error:error instanceof SeasonError?error.message:'The season service could not complete the request.'},error.status||500);}
  };
}
