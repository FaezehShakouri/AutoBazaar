...
import {createGame,prepareDay,settleDay,PRODUCTS,observation} from '../dist/engine.js';
...
test('model cache is frozen per season, private to engine, and validated',()=>{
  const m=neutral(),g=createGame({salesModel:m});m.calibration.products.water.baseSales=999;
...
test('placing additional orders cannot perturb future weather',()=>{
  const morning=prepareDay(createGame());
