...
const address=new URLSearchParams(location.search).get('address');if(/^0x[a-fA-F0-9]{40}$/.test(address||''))$('#agent-address').value=address;
try{const config=await api('config');button.disabled=!config.configured;button.textContent=config.configured?'Start Sandbox check →':'World ID setup pending';$('#check-description').textContent=config.credential==='selfie'?'Selfie Check Sandbox · No Orb needed. Complete a camera check on your phone. World must enable Selfie Check access for this app.':'Proof of Human Sandbox · Requires an Orb-verified credential.';if(!config.configured)status.textContent='The host needs to finish World ID setup before this check is available.';}catch{button.textContent='Connection unavailable';status.textContent='Could not reach the game server. Reload to reconnect.';}
...
async function verifyPending(){
  retry.hidden=true;
  try{const result=await api('verify',pendingVerification);pendingVerification=null;status.textContent=`✓ ${result.credential==='selfie'?'Selfie Check':'Proof of Human'} Sandbox proof verified for ${result.agentAddress}. Production season entry still requires AgentBook registration.`;}
